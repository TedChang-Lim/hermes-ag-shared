...
async fn connect_mimo(state: State<'_, AppState>, app: AppHandle) -> Result<String, String> {
    ...
    let mut child = Command::new(mimo_path)
        ...
        .env("MIMO_API_KEY", std::fs::read_to_string(format!("{}/.hermes/credentials/mimo_api_key.txt", std::env::var("HOME").unwrap_or_default())).unwrap_or_default().trim().to_string())
        .env("MIMO_BASE_URL", "http://127.0.0.1:1984/v1")
        .env("XIAOMI_API_KEY", std::fs::read_to_string(format!("{}/.hermes/credentials/mimo_api_key.txt", std::env::var("HOME").unwrap_or_default())).unwrap_or_default().trim().to_string())
