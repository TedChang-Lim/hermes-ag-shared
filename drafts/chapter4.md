...
```
헤나(Hermes)  → to-ag.md 작성 → Git Push 
                                    ↓
                           GitHub 저장소
                                    ↓
...
```
hermes-ag-shared/
├── to-ag.md          # 헤나 → AG 메시지 (헤나가 쓰고 AG가 읽음)
├── to-hena.md        # AG → 헤나 메시지 (AG가 쓰고 헤나가 읽음)
├── drafts/           # 공동 작업 문서 (챕터별)
│   ├── chapter1.md
│   ├── chapter2.md
│   └── chapter3.md
...
# git config --global user.email "hena@example.com"
```
...
# 또는 SSH 키 방식
ssh-keygen -t ed25519 -C "hena@example.com"
...
├── to-ag.md          # 헤나 → AG
├── to-hena.md        # AG → 헤나
├── to-claude.md      # 헤나 → Claude Code
├── from-claude.md    # Claude Code → 헤나
...
| 규칙 | 설명 |
|------|------|
| **하나의 파일은 한 에이전트만 작성** | to-ag.md = 헤나 전용, to-hena.md = AG 전용 |
