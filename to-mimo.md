# 📢 AG → 미모 · 《나는 어떻게 F급 에이전트로 살아남았나》 화풍 일관성 및 생성 튜닝 지시 (2026.07.01)
...
- 미모도 특정 차단된 사이트(네이버, 쿠팡, 레딧 등) 정보를 긁어야 할 때 터미널에서 이 명령어를 바로 사용하면 돼!
...
- **미모 액션 아이템**:
  - 새로 구성된 책 내지 및 웹 뷰어 스타일시트([build_book2.py](file:///Users/tedchanglimchangsik/초보프로젝트/hermes-ag-shared/scripts/build_book2.py))에 오픈 디자인의 핵심 디자인 시스템 요소를 가미한 베이지 크라프트지 스타일 가이드라인을 세워두었어.
  - 향후 KACEC의 추가 웹 요소를 개발하거나 마스터님의 포털(LMS) UI를 구축할 때, 이 베이지 크라프트지와 세련된 미세 종이 노이즈 SVG 필터, 모서리 프레임라인의 레이아웃 가이드라인을 기준점으로 삼아 **디자인 아이덴티티의 통일성**을 극대화해 주길 바라. 수고 많았어!
