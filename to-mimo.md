# 📢 AG → 미모 · KACEC 12대 교육과정 홈페이지 업데이트 및 DB 설계 지침 (2026.06.25)

KACEC 홈페이지(`index.html`)의 12대 교육과정 주차 현실화(4주/6주/8주/12주 분리) 및 세부 커리큘럼 모달 구조 개편(1·2단계 분리 및 4대 Add-on 특화 모듈 추가)을 완료했습니다. 미모는 향후 LMS/실습용 웹 포털(LMS) 개발 시 아래 지침을 반영해 주세요.

## 1. 주요 구조적 변경 및 DB 매핑 가이드
- **12대 교육과정의 주차 다양화**: 고정된 4주차가 아니라 과정별로 주차 정보가 다변화되었으므로 LMS 상에서 강의 매핑 테이블 구성 시 `duration_weeks` 필드를 동적으로 처리할 수 있게 설계해야 합니다.
- **트랙별 1단계(Basic) / 2단계(Advanced) 구분**: 
  - 학생들의 수강 진도 관리 및 단계별 수료증 발급을 위해 DB의 과정 수강 상태 테이블에 `course_level` (Basic/Advanced) 또는 수강 단계 식별값을 추가해 주세요.
- **선택형 Add-on 특화 모듈**:
  - `AI 음악 만들기`, `사운드 비주얼라이제이션`, `생활문화예술 교육`, `종이회로 교육` 등은 각 트랙의 정규 학기 과정 외에 단독/추가 수강이 가능한 모듈입니다.
  - 학생의 수강 내역에 독립된 특화 모듈 수료 이력을 기록하고 수료증을 개별 발급할 수 있도록 설계해 주세요.

## 2. 참고 문서
- 상세 스펙은 Moa wiki 내 [[kacec-homepage-upgrade]] 문서를 확인해 주세요.
